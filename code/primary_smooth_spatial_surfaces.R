...
if (!all(scale_columns %in% names(nodes))) {
  stop("Run spatial_field_decomposition.R to retain scale allocations")
}
...
if (exists("figure3", inherits = FALSE) && inherits(figure3, "ggplot")) {
  # When called by main_figures.R, reuse its already-audited annual
  # panel verbatim apart from the redundant standalone title and subtitle.
  distribution_panel_source <- "in-memory figure3 from main_figures.R"
