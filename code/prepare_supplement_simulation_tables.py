...
YEAR_FILE = PROJECT / "results" / "year_inference_simulation_summary.csv"
JOINT_FILE = PROJECT / "results" / "joint_dgp_simulation_summary.csv"
...
PORTABLE_TEX = MANUSCRIPT_ROOT / "generated" / "supp_complete_simulation_tables.tex"
...
def make_year_tables(frame: pd.DataFrame) -> list[str]:
    ...
    lines += [
        ...
        r"\setlength{\tabcolsep}{4.5pt}",
        r"\begin{longtable}{rrllrrrrr}",
        r"\caption{Complete 108-cell repeated-summer results: one-sided rejection percentages. For Student, lag-2 and growing-lag tests, null rows estimate mean-null size and non-null rows estimate power. The sign calculation targets $\Pr(Y_r<0)=1/2$; under centred skewed mean-null innovations, its rejection rate quantifies departure from the sign null. The three-test column requires Student, lag-2 and sign values all to be at most 0.025.}\label{tab:year-simulation-complete-c}\\",
        r"\toprule",
...
def main() -> None:
    ...
    PORTABLE_TEX.parent.mkdir(parents=True, exist_ok=True)
    year = pd.read_csv(YEAR_FILE, keep_default_na=False)
    joint = pd.read_csv(JOINT_FILE, keep_default_na=False)
    text = "\n".join([
        "% Generated by code/prepare_supplement_simulation_tables.py",
    ...
    print(
        f"Generated {PORTABLE_TEX} with {len(year)} annual and "
        f"{len(joint)} joint cells"
    )
