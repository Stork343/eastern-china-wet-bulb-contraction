...
SUBMISSION_ROOT = (
    PROJECT_DIR / "JRSSC"
    if (PROJECT_DIR / "JRSSC").is_dir()
    else PROJECT_DIR
)
DATA_DIR = PROJECT_DIR / "data" / "era5_confirmatory"
OUTPUT_DIR = PROJECT_DIR / "results"
PDF_FILE = SUBMISSION_ROOT / "submission" / "main_manuscript.pdf"
SUPPLEMENT_PDF_FILE = SUBMISSION_ROOT / "submission" / \
    "supplementary_material.pdf"
MANUSCRIPT_FILE = SUBMISSION_ROOT / "manuscript" / "main.tex"
SUPPLEMENT_FILE = SUBMISSION_ROOT / "manuscript" / "supplement_theory.tex"
FIGURE_OUTPUT_DIR = PROJECT_DIR / "results"
...
def main() -> None:
    ...
    audit = pd.read_csv(PROJECT_DIR / "results" /
    ...
    smooth_surface_audit = pd.read_csv(
        FIGURE_OUTPUT_DIR / "primary_smooth_map_surface_audit.csv"
    )
    smooth_build_audit = json.loads((
        FIGURE_OUTPUT_DIR / "primary_smooth_map_build_audit.json"
    ).read_text())
    ...
    sensitivity_dir = OUTPUT_DIR / "revision_sensitivity"
    sensitivity_required = [
        "revision_sensitivity_summary.csv",
        "extended_spatial_scale_curves.csv",
        "extended_spatial_profile_summary.csv",
        "extended_domain_masks.csv",
        "extended_climatology_loso_scale_curve.csv",
        "extended_climatology_loso_profile_summary.csv",
        "extended_station_scale_uncertainty.csv",
        "extended_station_availability.csv",
        "extended_station_record_effects.csv",
    ...
    require(
        ...
        sha256(PROJECT_DIR / "code" / "revision_sensitivity_analyses.py"),
    ...
    spatial_sensitivity = pd.read_csv(
        sensitivity_dir / "extended_spatial_profile_summary.csv"
    ).set_index("analysis")
    spatial_expected = {
    ...
    loso = pd.read_csv(
        sensitivity_dir / "extended_climatology_loso_scale_curve.csv"
    )
    loso_broad = loso.loc[
    ...
    convergence = pd.read_csv(
        PROJECT_DIR / "results" / "extended_spatial_convergence.csv"
    )
    ...
    graph_sim = pd.read_csv(
        PROJECT_DIR / "results" / "graph_simulation_summary.csv",
    ...
    joint_sim = pd.read_csv(
        PROJECT_DIR / "results" / "joint_dgp_simulation_summary.csv",
        keep_default_na=False,
    )
    ...
    dense_scale_identity = pd.read_csv(
        PROJECT_DIR / "results" / "dense_scale_identity_check.csv"
    )
    dense_identity = pd.read_csv(
        PROJECT_DIR / "results" / "dense_spatial_identity_check.csv"
    )
    ...
    portable_figures = {
        FIGURE_OUTPUT_DIR / "fig4_simulation_diagnostics.pdf":
            "figure01_simulation_diagnostics.pdf",
        PROJECT_DIR / "results" / "fig5_study_area.pdf":
            "figure02_study_area.pdf",
        FIGURE_OUTPUT_DIR / "fig3_multiscale_evidence.pdf":
            "figure03_multiscale_evidence.pdf",
        FIGURE_OUTPUT_DIR / "fig_primary_spatial_decomposition.pdf":
            "figure04_primary_spatial_decomposition.pdf",
        FIGURE_OUTPUT_DIR / "fig_energy_decomposition.pdf":
            "figure05_energy_decomposition.pdf",
        FIGURE_OUTPUT_DIR / "fig5_application_robustness.pdf":
            "figure06_application_robustness.pdf",
        FIGURE_OUTPUT_DIR / "fig6_noaa_agreement.pdf":
            "figure07_noaa_agreement.pdf",
        FIGURE_OUTPUT_DIR / "supp_dense_bandwidth_profile.pdf":
            "supp_dense_bandwidth_profile.pdf",
        FIGURE_OUTPUT_DIR / "supp_spatial_convergence.pdf":
    ...
    for generated_path, submission_name in portable_figures.items():
        ...
        portable_path = (SUBMISSION_ROOT / "manuscript" / "figures" /
                         submission_name)
        require(
