#!/usr/bin/env python3
"""Download and assemble the frozen ERA5-Land confirmatory point panel.

The CDS time-series product is optimized for one point per request. This
script therefore retrieves the 121 frozen locations concurrently, trims each
continuous 1991--2025 point series to the prespecified JJA buffers, and then
assembles one compact NetCDF panel per year for script 22.

Raw archives and trimmed point files are retained so interrupted runs resume
without resubmitting completed requests.
"""
...
BASE_DIR = PROJECT_DIR / "data" / "era5_confirmatory"
RAW_DIR = BASE_DIR / "cds_point_archives"
SLIM_DIR = BASE_DIR / "trimmed_points"
OUTPUT_DIR = BASE_DIR / "hourly_points"
PROVENANCE_FILE = BASE_DIR / "cds_point_provenance.json"

DATASET = "reanalysis-era5-land-timeseries"
ANALYSIS_YEARS = tuple(range(1991, 2026))
DISCOVERY_YEARS = (2015, 2022)
DATE_RANGE = "1991-05-31/2025-09-01"
CORE_VARIABLES = (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Concurrent CDS requests; the time-series service normally permits one.",
    )
...
def download_archive(site: pd.Series, variables: tuple[str, ...],
                     overwrite: bool, max_attempts: int) -> Path:
    import cdsapi
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            try:
                ...
                # cdsapi 0.7.7 delegates to LegacyClient, which removed the
                # retry keywords accepted by earlier releases. The outer loop
                # below supplies the same bounded retry behavior.
...
def trim_archive(site: pd.Series, archive: Path, variables: tuple[str, ...],
                 overwrite: bool) -> Path:
    import xarray as xr
    ...
    with tempfile.TemporaryDirectory(prefix=f"era5-site-{site_id:03d}-") as tmp:
        extraction = Path(tmp)
    ...
    missing = {"d2m", "t2m", "sp"} - set(panel.data_vars)
    if missing:
        raise ValueError(f"Site {site_id} lacks core variables {sorted(missing)}")
    wanted_names = {"d2m", "t2m", "sp"}
    if len(variables) > len(CORE_VARIABLES):
        wanted_names = EXPECTED_NAMES
    missing = wanted_names - set(panel.data_vars)
    if missing:
    ...
    panel = panel.isel(time=positions)[sorted(wanted_names)]
    if panel.sizes.get("time") != 2232 * len(ANALYSIS_YEARS):
...
def process_site(site: pd.Series, variables: tuple[str, ...], overwrite: bool,
                 assemble_only: bool, max_attempts: int) -> Path:
    site_id = int(site.site_id)
    ...
    if assemble_only:
        ...
        archive = download_archive(site, variables, overwrite, max_attempts)
    output = trim_archive(site, archive, variables, overwrite)
...
def assemble_year(site_files: list[Path], sites: pd.DataFrame, year: int,
    ...
    for path, site in zip(site_files, sites.itertuples(index=False)):
        ...
        piece = piece.expand_dims(site=[int(site.site_id)])
        pieces.append(piece)
    ...
    if panel.sizes.get("time") != 2232 or panel.sizes.get("site") != 121:
...
def main() -> None:
    ...
    variables = CORE_VARIABLES if args.core_only else \
        CORE_VARIABLES + MECHANISM_VARIABLES
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(process_site, row, variables, args.overwrite,
        ...
        "archives": [
            {
                ...
                "bytes": (RAW_DIR / f"site_{int(site.site_id):03d}.zip")
                         .stat().st_size,
                "sha256": sha256(
                    RAW_DIR / f"site_{int(site.site_id):03d}.zip"
                ),
            }
