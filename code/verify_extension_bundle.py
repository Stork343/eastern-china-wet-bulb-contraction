...
MANUSCRIPT_ROOT = (
    PROJECT / "JASA" / "manuscript"
    if (PROJECT / "JASA" / "manuscript").is_dir()
    else PROJECT / "manuscript"
)
HIST_DATA = PROJECT / "data" / "era5_historical_extension"
HIST_OUT = PROJECT / "results"
METHOD_OUT = PROJECT / "results"
ELEV_OUT = PROJECT / "results"
NOAA_DATA = PROJECT / "data" / "noaa_isd_extension"
NOAA_ERA = NOAA_DATA / "era5_land_points"
NOAA_OUT = PROJECT / "results"
PRIMARY_SITE_FILE = PROJECT / "data" / "grid" / "eastern_china_121_sites.csv"
MAIN_TEX = MANUSCRIPT_ROOT / "main.tex"
SUPPLEMENT_TEX = MANUSCRIPT_ROOT / "supplement_theory.tex"
...
def verify_complete_simulation_tables() -> None:
    annual = read_csv(PROJECT / "results" /
    ...
    joint = read_csv(PROJECT / "results" /
    ...
    portable_tex = need(MANUSCRIPT_ROOT / "generated" /
...
def verify_cross_record_dependence_stress() -> None:
    ...
    need(PROJECT / "code" / "cross_record_dependence_stress.py",
...
def verify_noaa_analysis(qualification: pd.DataFrame,
    ...
    events = read_csv(PROJECT / "results" / "sensitivity_event_manifest.csv")
...
def verify_post_review_sensitivities() -> None:
    ...
    output_dir = PROJECT / "results" / "revision_sensitivity"
    audit = read_json(
        output_dir / "revision_sensitivity_audit.json",
        "post-review sensitivity audit",
    )
    script = need(PROJECT / "code" / "revision_sensitivity_analyses.py")
    ...
    spatial = read_csv(output_dir / "extended_spatial_profile_summary.csv")
    ...
    curves = read_csv(output_dir / "extended_spatial_scale_curves.csv")
    ...
    loso = read_csv(output_dir / "extended_climatology_loso_scale_curve.csv")
    ...
    station = read_csv(output_dir / "extended_station_scale_uncertainty.csv")
    ...
    availability = read_csv(output_dir / "extended_station_availability.csv")
