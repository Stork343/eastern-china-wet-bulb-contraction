...
# cyclic-t3 day-level field mechanism from graph_esh_simulation.R.  Each
...
simulate_base_record <- function(n) {
  ...
  # graph_esh_simulation.R.  Its square makes graph energy heavy tailed.
...
tex <- c(
  "% Generated by code/ratio_stress_test.R",
...
audit <- list(
  ...
  outputs = lapply(outputs, function(path) list(
    path = file.path("results", basename(path)),
